...
### 2.10. Anticipos de Sueldo y Préstamos Internos (`/api/salary-advances`)
- `GET /api/salary-advances`: Lista todas las solicitudes de anticipos con filtros por estado (`PENDING`, `APPROVED`, `REJECTED`, `PAID`). (Requiere `admin` o `hr`)
- `POST /api/salary-advances`: Empleado solicita un anticipo `{ amount, installments, reason }`. Valida tope mensual <= 50% sueldo base. (Autenticado)
- `PUT /api/salary-advances/:id/status`: Administrador aprueba o rechaza solicitud `{ status }`. (Requiere `admin` o `hr`)
- `GET /api/salary-advances/my`: Consulta historial de anticipos del empleado autenticado. (Autenticado)

### 2.11. Onboarding, Expediente Digital, Equipos y Offboarding (`/api/onboarding-offboarding`)
- `GET /api/onboarding-offboarding/expedient/my`: Obtiene expediente digital y % de completitud de onboarding del usuario activo. (Autenticado)
- `GET /api/onboarding-offboarding/expedient/:id`: Obtiene expediente digital y estado de verificación de documentos de un empleado. (Requiere `admin` o `hr`)
- `POST /api/onboarding-offboarding/expedient/upload`: Carga documento de expediente `{ type, documentCategory, documentUrl, mimeType, originalName }`. (Autenticado)
- `PUT /api/onboarding-offboarding/expedient/verify/:id`: Aprueba o rechaza un documento del expediente `{ status, notes }`. (Requiere `admin` o `hr`)
- `GET /api/onboarding-offboarding/assets`: Lista inventario de equipos, EPPs y herramientas asignados. (Requiere `admin` o `hr`)
- `GET /api/onboarding-offboarding/assets/employee/:employeeId`: Obtiene activos asignados a un empleado. (Autenticado)
- `POST /api/onboarding-offboarding/assets/deliver`: Registra entrega de activo/EPP `{ employeeId, name, serialNumber, category, condition }`. (Requiere `admin` o `hr`)
- `PUT /api/onboarding-offboarding/assets/return/:id`: Registra devolución de activo `{ returnNotes, condition, status }`. (Requiere `admin` o `hr`)
- `POST /api/onboarding-offboarding/offboarding/simulate`: Simula liquidación legal según causal (13er, 14to, vacaciones, desahucio, despido intempestivo). (Requiere `admin` o `hr`)
- `POST /api/onboarding-offboarding/offboarding/start`: Inicia proceso de salida oficial y genera checklist dinámico. (Requiere `admin` o `hr`)
- `PUT /api/onboarding-offboarding/offboarding/:id/checklist`: Marca tarea completada en el checklist de salida `{ taskId, completed }`. (Requiere `admin` o `hr`)
- `GET /api/onboarding-offboarding/offboarding`: Lista procesos de salida activos e historial de liquidaciones. (Requiere `admin` o `hr`)

### 2.12. Cumplimiento Legal y Provisiones Sociales (`/api/compliance`)
- `GET /api/compliance/alerts`: Genera matriz de alertas preventivas (fin período prueba 90d, vencimiento contratos plazo fijo, documentos caducados). (Requiere `admin` o `hr`)
- `GET /api/compliance/provisions`: Calcula matriz de provisiones mensuales patronales de ley (13ro, 14to SBU, Fondos de Reserva, Vacaciones) por departamento y empleado. (Requiere `admin` o `hr`)

### 2.13. Comunicados Internos y Acuse de Recibo Digital (`/api/announcements`)
- `GET /api/announcements`: Obtiene el feed de comunicados con estado de lectura y acuse del usuario autenticado. (Autenticado)
- `POST /api/announcements`: Publica nuevo comunicado oficial `{ title, content, category, priority, requiresAcknowledgment, attachmentUrl }`. (Requiere `admin` o `hr`)
- `POST /api/announcements/:id/read`: Registra lectura o firma digital de acuse de recibo `{ acknowledge }`. (Autenticado)
- `GET /api/announcements/:id/stats`: Métricas de lectura (% leídos, acuses firmados y lista de empleados pendientes). (Requiere `admin` o `hr`)
- `GET /api/announcements/birthdays`: Consulta colaboradores que cumplen años en el mes actual. (Autenticado)
