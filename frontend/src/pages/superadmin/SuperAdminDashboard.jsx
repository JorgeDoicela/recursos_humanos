        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-6 sm:p-7 rounded-2xl border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <button
                    ...
                    disabled={loading}
                    className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-4 py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-colors cursor-pointer shadow-xs disabled:opacity-50 self-start md:self-auto"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
                    ...
                    <div className="p-3 bg-slate-100 text-slate-700 rounded-xl text-xl">
                        <FiDollarSign />
                    </div>
                </div>
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-2xl font-bold text-slate-900 mt-1 tracking-tight">
                            {metrics?.activeTenants ?? 0} <span className="text-sm font-normal text-slate-400">/ {metrics?.totalTenants ?? 0}</span>
                        </p>
                        <p className="text-xs text-emerald-700 font-medium mt-0.5">Suscripciones pagadas</p>
                    </div>
                    <div className="p-3 bg-emerald-50 text-emerald-700 rounded-xl text-xl">
                        <FiCheckCircle />
                    </div>
                </div>
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
                    ...
                    <div className="p-3 bg-amber-50 text-amber-700 rounded-xl text-xl">
                        <FiClock />
                    </div>
                </div>
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
                    ...
                    <div className="p-3 bg-blue-50 text-blue-700 rounded-xl text-xl">
                        <FiUsers />
                    </div>
                </div>
            </div>
                ...
                <div className="p-5 border-b border-slate-200 space-y-4">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full sm:w-72 pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-400 text-slate-800 transition-all"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100">
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${activeTab === 'ALL' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                        >
                            Todas ({metrics?.totalTenants ?? 0})
                        </button>
                        <button
                            onClick={() => setActiveTab('ACTIVE')}
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${activeTab === 'ACTIVE' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                        >
                            Activas ({metrics?.activeTenants ?? 0})
                        </button>
                        <button
                            onClick={() => setActiveTab('TRIAL')}
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${activeTab === 'TRIAL' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                        >
                            En Trial ({metrics?.trialTenants ?? 0})
                        </button>
                        <button
                            onClick={() => setActiveTab('EXPIRING')}
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${activeTab === 'EXPIRING' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                        >
                            Vencen Pronto ({metrics?.expiringTrialsCount ?? 0})
                        </button>
                        <button
                            onClick={() => setActiveTab('SUSPENDED')}
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${activeTab === 'SUSPENDED' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                        >
